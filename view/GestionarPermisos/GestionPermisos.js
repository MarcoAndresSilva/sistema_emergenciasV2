document.addEventListener("DOMContentLoaded", function () {
    ...
    const table = $('#reglas-table').DataTable({
        responsive: true,
        ajax: {
            url: '../../controller/permisos.php?op=get_permisos',
            dataSrc: '',
            method: 'GET',
            cache: false
        },
        columns: [
            { title: 'ID Regla', data: 'id_permiso' },
            { title: 'Permisos', data: 'permiso' },
    ...
    $('#reglas-table tbody').on('click', '.edit-btn', async function () {
        ...
        try {
            const response = await fetch('../../controller/permisos.php?op=get_permisos');
            if (!response.ok) throw new Error('Network response was not ok');
            
            const rules = await response.json();
            const ruleData = rules.find(rule => rule.id_permiso == idRegla); // Find the rule with the specific id
...
async function updateRegla(data) {
    try {
        ...
        formData.append('id', data.id_permiso);
        ...
        const response = await fetch('../../controller/permisos.php?op=update_permisos', {
            method: 'POST',
            body: formData
        });
